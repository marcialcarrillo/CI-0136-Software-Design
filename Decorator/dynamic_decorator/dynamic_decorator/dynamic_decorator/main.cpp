...
int main() {
	...
	AristaDirectional aristaNonDirectional = new AristaDirectional(false);
	Arista& aristaAggDir = *(new AristaAggregation{ *(new AristaDirectional{true}) });
	AristaComposition aristaCompAsso{
		*(new AristaAssociation { *(new AristaDirectional{false}) })
	};
